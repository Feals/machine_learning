...
for model_name, best_model in best_models.items():
    ...
    # Appliquer le seuil ajusté de 0.35 (uniquement si predict_proba existe)
    if y_score is not None:
        seuil = 0.30
        y_pred_adjusted = (y_score >= seuil).astype(int)
    else:
    ...
    print(f"✅ Recall: {rec:.4f}  (détection des malades)")
    print(f"✅ Précision: {prec:.4f}")
    print(f"✅ F1-score: {f1:.4f}")
    print(f"✅ Balanced Accuracy: {bal_acc:.4f}")
    print(f"✅ Spécificité: {specificity:.4f}  (détection des non-malades)")
